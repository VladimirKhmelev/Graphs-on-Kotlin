...
class GraphViewModel {
    ...
    // Настройка изменения графа(ребра + вершины)
    val graphSettings = mutableStateOf(GraphSettings())

    // Подсказки для режимов
    var vHintVisible by mutableStateOf(false)
    var vHintMessage by mutableStateOf("")
    var dHintVisible by mutableStateOf(false)
    var dHintMessage by mutableStateOf("")
    ...
    // Обновление настроек графа - вершин и рёбер
    fun updateSettings(newSettings: GraphSettings) {
        graphSettings.value = newSettings
        // Форсируем обновление графа
        _graph.value = _graph.value?.copy()
    }

    // Установка результата алгоритма с таймером
    fun setAlgorithmResult(result: AlgorithmResult?) {
        ...
        // Если результат не null, запускаем таймер скрытия
        if (result != null) {
    ...
    // Очистка графа
    fun clearGraph() {
        setAlgorithmResult(null) // Сбрасываем результат
        saveState()
        _graph.value = Graph()
        selectedStart = null
        selectedEnd = null
    ...
    fun runAlgorithm(algorithm: AlgorithmType) {
        setAlgorithmResult(null) // Сбрасываем предыдущий результат
        saveState()
        val currentGraph = _graph.value ?: run {
            _uiState.value = UiState.Error(tr("graph_empty_error"))
            return
        }
            ...
            try {
                val result = when (algorithm) {
                    ...
                    AlgorithmType.FORCEATLAS2 -> {
                        applyForceLayout()
                        AlgorithmResult.MessageResult(tr("layout_suc"))
                    }
                }
                withContext(Dispatchers.Main) {
                    setAlgorithmResult(result) // Используем функцию с таймером
                    _uiState.value = UiState.Success("${algorithm.name} executed")
                }
            } catch (e: Exception) {
                withContext(Dispatchers.Main) {
                    _uiState.value = UiState.Error(e.message ?: tr("algorithm_failed"))
                }
            }
        }
    }
    ...
    private fun applyForceLayout() {
        ...
        _graph.value?.let { graph ->
            ...
            _uiState.value = UiState.Success(tr("layout_suc"))
        } ?: run {
            _uiState.value = UiState.Error(tr("empty_graph"))
            setAlgorithmResult(AlgorithmResult.MessageResult(tr("layout_suc")))
        }
    }
    ...
    // Поиск подходящих точек для изменения веса ребра
    ...
    fun updateEdge(oldEdge: Edge, newWeight: Double, isDirected: Boolean) {
        ...
        _graph.value?.let { graph ->
            ...
            val newEdge = if (isDirected) {
                DirectedWeightedEdge(oldEdge.from, oldEdge.to, newWeight)
            } else {
                WeightedEdge(oldEdge.from, oldEdge.to, newWeight)
            }
    ...
    // Функция для поддержки изменения языка
    fun tr(key: String) = Localization.tr(key)
    ...
    private fun handleDijkstra(currentGraph: Graph<Edge>): AlgorithmResult {
        ...
        return if (result != null) {
            ...
            AlgorithmResult.MessageResult(tr("path_not_found"))
        }
    }
